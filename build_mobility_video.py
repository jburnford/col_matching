#!/usr/bin/env python3
"""Build the data + HTML for a 2-minute time-lapse of EVERY career transfer in the KG,
showing the scale of mobility in the British Empire. Canvas animation over a Leaflet/CARTO
basemap: a clock runs 1830->1966, each transfer flies as a glowing arc and settles into a
persistent faint web. Frames captured by render_mobility.py, encoded to MP4."""
import json

data = json.load(open("/tmp/transfers.json"))
coords = json.load(open("/tmp/transfer_coords.json"))
...
print(f"{len(arcs):,} mapped transfers, {y0}-{y1}")

HTML = """<!DOCTYPE html><html><head><meta charset="utf-8">
<link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css"/>
<script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
<style>
 html,body{margin:0;height:100%%;background:#070f18;overflow:hidden;font-family:Georgia,serif}
 #map{height:100%%;background:#070f18}
 ...
 #hud p{margin:6px 0 0;font-size:16px;color:#dfe7ef;font-family:system-ui,sans-serif;max-width:560px;line-height:1.35}
 ...
 #year .c{font-size:17px;color:#cfdae6;margin-top:2px}
...
<div id="hud"><h1>Moving the Empire</h1>
 <p>Every recorded career transfer of a British colonial official, 1830s&ndash;1960s &mdash;
    one official changing colony, one arc. 16,000+ moves by 8,000 officials.</p></div>
<div id="year"><div class="y" id="yr">1835</div><div class="c"><span id="cn">0</span> transfers</div></div>
<div id="credit">Colonial Office List career data &middot; Wikidata coordinates</div>
<div id="map"></div>
<canvas id="cv"></canvas>
<script>
...
let PIX=null;
function project(){ // colony lat/lon -> container pixels (view is fixed)
...
const FLIGHT=1.4;                 // seconds an arc spends "flying"
let lastDrawn=0;                  // index into ARCS already burned into persistent web
let curYear=Y0;
const yearToTime = y => (y - Y0);  // years since start (clock units == years)
...
window.renderFrame=function(tYears){
 ...
 while(lastDrawn<TOTAL && ARCS[lastDrawn][0] + FLIGHT <= nowYear){
  ...
  pctx.strokeStyle='rgba(150,190,255,0.07)'; pctx.lineWidth=0.7;
 ...
 for(let i=lastDrawn;i<TOTAL;i++){
  ...
  ctx.strokeStyle='rgba(255,210,140,'+(0.55*(1-f)+0.12)+')'; ctx.lineWidth=1.2;
  ...
  ctx.beginPath(); ctx.fillStyle='rgba(255,245,210,0.95)'; ctx.arc(hd[0],hd[1],2.4,0,7); ctx.fill();
 }
...
open("docs/mobility_anim.html", "w").write(HTML % {"arcs": json.dumps(arcs), "y0": y0, "y1": y1})
print("wrote docs/mobility_anim.html")
