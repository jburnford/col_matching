#!/usr/bin/env python3
"""GLOBAL (empire-scale) IOL mobility video -- a faithful replica of the Colonial
Office build_mobility_video.py / mobility_anim.html, but driven by the India Office
List transfer data.  Same world view (setView([14,12],2.3)), same blue accumulating
web, same gold clock -- so it sits as a MATCHED PAIR beside the CO empire_mobility.mp4
for direct visual contrast: the CO graph fills the globe; the IOL graph concentrates
into a dense Indian knot with sparse threads to Britain, Africa, Canada and the Gulf.

Frames captured by render_iol_mobility_global.py, encoded to MP4."""
import json
...
<div id="hud"><h1>Moving the Indian Empire</h1>
 <p>Every recorded career transfer of an India Office official, 1820s&ndash;1940s &mdash;
    one officer moving between presidency, province or princely state, one arc. 9,000+ moves
    by 4,000 officials, on the same world stage as the Colonial Office.</p></div>
...
open("docs/iol_mobility_global_anim.html", "w").write(HTML % {"arcs": json.dumps(arcs), "y0": y0, "y1": y1})
print("wrote docs/iol_mobility_global_anim.html")
