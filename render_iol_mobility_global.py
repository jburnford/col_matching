...
import pathlib, os
os.makedirs("/tmp/iolframes_g", exist_ok=True)
for f in pathlib.Path("/tmp/iolframes_g").glob("*.png"): f.unlink()
url = "file://" + str(pathlib.Path("docs/iol_mobility_global_anim.html").resolve())
...
with sync_playwright() as p:
    ...
    for k, ty in enumerate(frames):
        ...
        pg.screenshot(path=f"/tmp/iolframes_g/f{k:05d}.png")
    b.close()
